...
describe( 'reducer', () => {
	...
	it( 'should store the current query', () => {
		...
		expect( newState ).to.eql( { 123: { currentPage: 2, currentSearch: '', currentProduct: null } } );
	} );
	...
	it( 'should update the current search when changed', () => {
		...
		const originalState = deepFreeze( { 123: { currentPage: 3, currentSearch: '' } } );
		const newState = reducer( originalState, action );
		expect( newState ).to.eql( { 123: { currentPage: 3, currentSearch: 'testing', currentProduct: null } } );
	} );
	...
	it( 'should update the current product when changed', () => {
		...
		const originalState = deepFreeze( { 123: { currentPage: 3, currentSearch: '' } } );
		const newState = reducer( originalState, action );
		expect( newState ).to.eql( { 123: { currentPage: 3, currentSearch: '', currentProduct: 50 } } );
	} );
	...
	it( 'should store the current query for more than one site', () => {
		...
		const originalState = deepFreeze( { 123: { currentPage: 3, currentSearch: '', currentProduct: null } } );
		const newState = reducer( originalState, action );
		expect( newState ).to.eql( {
			123: { currentPage: 3, currentSearch: '', currentProduct: null },
			234: { currentPage: 1, currentSearch: 'testing', currentProduct: null }
		} );
	} );
